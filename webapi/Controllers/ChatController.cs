...
using CopilotChat.WebApi.Constants;
...

{
    ...
    [FromQuery] string deploymentName = "gpt-4")

    {

        this._logger.LogInformation("Chat request received from Username: {0} UserID: {1} Email: {2} DateTime: {3} Question: {4}", authInfo.Name, authInfo.UserId, authInfo.Email, DateTime.Now.ToString(), ask.Input);
        ...
        properties.Add(TrackingEventProperties.ConversationId, chatIdString);

        properties.Add(TrackingEventProperties.Email, authInfo.Email);

        properties.Add(TrackingEventProperties.UserId, authInfo.UserId);

        properties.Add(TrackingEventProperties.Query, ask.Input);

        this._telemetryService.TrackEvent(TrackingEventNames.UserQuery, properties);
    ...
    private static KernelArguments GetContextVariables(Ask ask, IAuthInfo authInfo, string chatId, string deploymentName = "gpt-4")
